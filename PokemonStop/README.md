...
3. Duplicate `.env.example` to `.env` and set up your database credentials.
4. Execute `php artisan key:generate`.
5. Run migrations and seeders: `php artisan migrate --seed`.
6. Publish storage link: `php artisan storage:link`.
7. Optional NPM build: `npm install && npm run dev`.
8. Start the server with `php artisan serve` and visit `http://localhost:8000`.
